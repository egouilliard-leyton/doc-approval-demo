"""Generic vision-language-model OCR adapter (transcription over OpenRouter).

One :class:`VLMEngine` instance = one OpenRouter model. Each page PNG is sent to a
multimodal model over OpenRouter's OpenAI-compatible API, which transcribes it to
faithful Markdown (text + tables preserved). Connecting a new VLM is just a
different ``model`` slug — no new subclass needed (the DB registry builds these
directly from ``VlmEngineRow`` rows).
...
honestly rather than faked). Docling stays the spatially-grounded engine; a VLM
...
class VLMEngine(OCREngine):
    """OCR via any OpenRouter multimodal model, identified by (name, model)."""

    def __init__(self, name: str, model: str, device: str | None = None) -> None:
        super().__init__(device)
        self.name = name
        self.model = model
        self.version = model  # the model slug doubles as the engine version stamp

    def _client(self):
        if not settings.openrouter_api_key:
            raise ValueError(
                f"OPENROUTER_API_KEY is not set; the '{self.name}' OCR engine needs it."
            )
    ...
    def _transcribe(self, client, path: Path) -> str:
        ...
        try:
            ...
            raise ValueError(f"{self.name} OCR request failed: {exc}") from exc
    ...
    def _ocr_pages(self, doc_id: str, pages: list[Path]) -> tuple[list[OCRPage], list[str]]:
        ...
        return out, [f"{self.name} does not expose bounding boxes or per-block confidence"]
