...
| [document-generation.md](./document-generation.md) | **Document generation from templates** — turn an extraction into a filled DOCX/PDF/**Excel (.xlsx)**; the three modes (form-fill / rich-HTML / **spreadsheet** with visual cell mapping + LibreOffice-computed preview), the AI authoring agent, vision Fidelity QA, revision history, the data model + endpoints, and the honest TipTap-flattening limitation. |
