...
- **Fuzzy vs. exact — DECIDED: we ship both, as configuration.** Every comparison-style validation (equality, cross-doc consistency, address/name agreement) exposes a **match mode**, and fuzzy matching has a **tunable threshold slider**. The author is never forced to pick fuzzy *or* exact — they dial it. Crucially, the builder UI must show **live examples for wherever the slider sits**, so the author sees what a given threshold actually accepts/rejects instead of guessing at an abstract number. Sketch of the slider and the examples it would surface (using name matching):
